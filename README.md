...
- **Slash commands** (`/help`, `/code`, `/review`, `/deep`, `/file`, `/summarize`, `/skill-creator`…) parsed on-device before a request is sent.
- **Skill creator** — `/skill-creator <brief>` compiles a named, reviewable custom skill (purpose, operating procedure, guardrails). You confirm before save. Skills only shape wording; they never grant tools, network, or phone access.
...
8. Open **Skills & language** from the drawer or Settings to select fixed response skills and a JavaScript, TypeScript, Kotlin, Java, or Linux shell artifact preset. Type `/skill-creator` plus a brief to compile a custom skill; review the card before it is saved. Skills are included in the next request's system guidance and never grant tools.
